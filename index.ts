...
let rand:number=Math.floor(Math.random()*9+1)
let numTries:number=3
let play:boolean=true
let actualGuess:number=rand
interface AnswersType{
    MyGuess:number
}
interface playagnType{
  playagain:boolean
}
while(play){ 
...
if(answer.MyGuess==actualGuess){
    ...
    if(actualGuess>answer.MyGuess){console.log("think higher")}
    else{
...
const answerplay:playagnType=await inquirer.prompt([{
    name:'playagain',
    type:'confirm',
